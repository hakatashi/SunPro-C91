...
[1]では@<img>{pix2pix}@<fn>{origimg}のように白黒写真のカラー化から、航空写真からの地図の生成、また、
...
pix2pixは、これを発展させた「条件付き敵対的生成ネットワーク(Conditional GAN)」を使用しています。
Conditional GANでは、航空写真と地図、白黒写真とカラー写真のように入力と出力に対応する2つの画像のペアを学習データとして用います。
そしてGeneratorには、航空写真や白黒写真といった入力側となる学習データを与え、それに合った出力画像を生成するようにします。
さらにDiscriminatorには、学習データに含まれる正しいペアと、
入力側だけ学習データで出力側はGeneratorが生成した画像という偽のペアを与え、判別させます。
つまり@<img>{cgan}のようになり、これによって画像変換をGANの仕組みの中で扱えるようになります。
...
つまり、ペアとなった画像の学習データさえ用意すれば、ここまで説明してきた技術をすぐに使えるのです。
...
これを見ると、リキテンスタインの絵の特徴的なポイントは、以下の3点にあることが分かります。

 * 太く黒い線でしっかりと縁取られた輪郭線
 * 赤や黄といった原色に近い鮮やかな配色
 * 斜線や水玉といった幾何学的かつ単純なテクスチャ

逆に考えると、写真とそれに写っている物体の輪郭の情報があれば、
機械的に配色し、テクスチャを貼り付けることでリキテンスタイン風の絵を生成できそうです。

さらに、輪郭抽出はCanny法と呼ばれるアルゴリズムを使えば、OpenCVなどで自動的にできるのでどうにかなるように思えます。
しかし、Canny法で複雑な画像に対して輪郭抽出をしようとすると、かなり細かくパラメータ設定をしないと
@<img>{canny}のように線が非常に多くなってしまうので、あまりリキテンスタインっぽくはならなさそうです。
...
実際にデータを見てみると@<img>{nyudepth}のように、少しリキテンスタインのような雰囲気が見て取れるのが分かります。
...
まず4行目のforによって、画像に含まれる物体のそれぞれについて同じ処理を繰り返すようにしています。
そして、リキテンスタインの絵画では物体がそれぞれ1色で塗られていることから7~12行目で物体を塗りつぶす色を決定しています。
ここでは、物体の領域に含まれる色の中央値を取った上で、原色に近い色にするためにRGBをそれぞれ1.3倍するという処理を行っています。

そこから、15~25行目で決定した色のテクスチャをランダムに生成しています。
ここで@<code>{fill(shape, color)}は与えられたサイズを与えられた色で埋めた画像、
@<code>{dot(shape, color)}は水玉で埋めた画像、@<code>{diagonal(shape, color)}は斜線で埋めた画像を生成する関数とします。
つまり、後で物体に該当する部分だけ切り抜いて使うべく、画像サイズと同じテクスチャを作っておくという処理になっています。
またリキテンスタインの絵画では、壁や床などの大きな領域では幾何学的なテクスチャがあまり用いられていないことから、
...
テクスチャを生成する関数は、以下の通りです。
詳細な説明は割愛しますが、水玉のサイズや斜線の傾きはランダムで、描画にはPILの@<code>{ImageDraw}を使っています。
...
//image[royconv][元の写真と生成されたロイ・リキテンスタイン風の変換画像の例][scale=0.96]
以上を実行すると@<img>{royconv}のように写真とリキテンスタイン風の変換画像のセットが生成されます。
...
そして、最終的に写真風に変換する、リキテンスタインのポップアートもデータセットに加えておきます。
そのためにまず、以下のソースコードを用いて@<m>{640 \times 480}の解像度で切り出します。
...
これを@<code>{crop.py}として以下の操作を行います。
なお、これらは学習に用いず、また正解データも存在しないのですが、
pix2pixはその場合でもペアとなった画像を入力とするので、
...
最後にGitHubの実装にある@<code>{combine_A_and_B.py}によって結合処理をしておく必要があります。
...
結果は@<img>{rescolor}で、左側のテストデータでの出力画像を見ると、それなりに元の写真に近いものが出力されていることが分かります。
