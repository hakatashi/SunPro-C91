...
以下2点の誤字脱字を訂正いたしました。
   ...
   *  「1.1 はじめに」内 「iTunesで視聴可能なので」 → 「iTunesで試聴可能なので」
   ＊  「1.3 各メンバーの声を集める」内の「リスト1.4: wavに変換し音声分離」のコードの28行目 @<tt>{ffmpeg -i} → @<tt>{ffmpeg -i \\}
...
数ある声優ユニットの中からμ'sを選んだのには、以下の理由があります:
    ...
    * メンバー1人のみが歌っている「ラブライブ！ Solo Live!」というアルバムが各メンバーごとにあり、iTunesで試聴可能なので、サンプル音声の収集が楽そう
