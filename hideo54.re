...
この波形(=声の特徴)を扱いやすくするために、メル周波数ケプストラム係数(MFCC)というものを使うと良いらしいです。波形をフーリエ変換することで得られるスペクトル@<fn>{fyi}の対数値をさらにフーリエ変換して得られたケプストラムを、メル尺度を使って変換したものだとか。なるほどわからん。
...
ついでに色々やっているので、少し説明:
    ...
    * もぎゅっとloveで接近中のみ、試聴曲30秒のうち半分は歌ってないので、後半15秒のみにカット
...
集めた99曲のうち"No brand girls"を除いた90曲のMFCCを、scikit-learnを使って、SVM(サポートベクトルマシン)を用いて分類します。@<fn>{why-svm}
